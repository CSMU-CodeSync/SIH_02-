# Security Audits, SLA Compliance & Verification Rules (Security_audits.md) — Full-Stack
...
Special focus is given to auditing the **SRCS (Stage Resolve Commit System)** SLA transitions (24h/36h/72h) and verifying resolution proofs via the **PRR (Problem Resolve Re-evaluation) Engine** and **PRRStudio React Component**.

---

## 2. System Security Audit Topology

Security auditing monitors every layer of the system architecture from React UI components to NGINX load balancing and external State/Central DBMS persistence:
...
| Vulnerability | Threat Description | Full-Stack Mitigation |
| :--- | :--- | :--- |
| **A01: Broken Access Control** | Unauthorized access to department endpoints (`dep_01..03`) | Role-Based Access Control (RBAC) middleware verifying session role claims in Redis + React Auth Route Guards. |
| **A02: Cryptographic Failures** | Plaintext transmission of citizen grievances | Mandatory SSL/TLS 1.3 at NGINX, Client & Server AES-256-GCM Encryption Barrier, and HMAC-SHA256 Salting Barrier. |
| **A03: Injection** | SQL Injection via raw query parameters | Mandatory SQLAlchemy 2.0 ORM parameterization; zero raw SQL strings. |
| **A04: Insecure Design** | Premature or fake complaint resolution by officers | **SRCS PRR Engine**: Mandatory proof checking with Gemini 2.5 Flash vision/text context match in `PRRStudio.tsx`. |
| **A05: Security Misconfiguration** | Exposed debug modes, default secret keys | Environment variables validated at boot via Flask config schema and Vite `import.meta.env`. |
| **A07: Identification Failures** | Brute force session hijacking | Redis Sliding Window Rate Limiting (max 60 req/min per IP, 5 login attempts/min) + Axios exponential backoff toast. |

---

## 4. SRCS SLA Audit & Escalation Engine Service (`app/features/srcs/tasks.py`)

```python
from datetime import datetime, timezone
from app.features.srcs.models import Complaint
